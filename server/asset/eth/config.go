...
; jwt hex secret shared with a geth full node when connecting remotely over websocket. Not needed for ipc
...
type config struct {
	...
	// communication.
	JWT string `long:"jwt" description:"The jwt secret needed to connect to a geth full node if connecting over websocket."`
}
...
func loadConfig(configPath string, net dex.Network, logger dex.Logger) (*config, error) {
	...
	if !strings.HasSuffix(cfg.ADDR, ".ipc") && cfg.JWT == "" {
