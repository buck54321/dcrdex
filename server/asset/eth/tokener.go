...
func newTokener(ctx context.Context, assetID uint32, net dex.Network, be bind.ContractBackend) (*tokener, error) {
	token, tokenAddr, contractAddr, err := networkToken(assetID, net)
	if err != nil {
		return nil, err
	}
	...
	es, err := erc20v0.NewERC20Swap(contractAddr, be)
	if err != nil {
		return nil, err
	}

	erc20, err := erc20.NewIERC20(tokenAddr, be)
	if err != nil {
		return nil, err
	}
	...
	if boundAddr, err := tkn.tokenAddress(ctx); err != nil {
		...
		return nil, fmt.Errorf("wrong bound address for %s version %d contract. wanted %s, got %s",
			token.Name, token.ver, tokenAddr, boundAddr)
	}
