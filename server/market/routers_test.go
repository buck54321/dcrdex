...
func (m *tMatchNegotiator) AccountStats(acctAddr string, assetID uint32) (qty, lots uint64, redeems int) {
	return m.qty, m.lots, m.redeems
}
...
func TestMain(m *testing.M) {
	...
	oRig.router = NewOrderRouter(&OrderRouterConfig{
		...
		AccountBalancer: NewBackedBalancer(tunnels, assets, matchNegotiator),
	})
	rig = newTestRig()
	src1 := rig.source1
	src2 := rig.source2
