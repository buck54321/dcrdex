...
type MatchNegotiator interface {
	AccountStats(acctAddr string, assetID uint32) (qty, lots uint64, redeems int)
}
...
func NewOrderRouter(cfg *OrderRouterConfig) *OrderRouter {
	router := &OrderRouter{
		...
		accountBalancer: cfg.AccountBalancer,
	}
...
func (r *OrderRouter) handleLimit(user account.AccountID, msg *msgjson.Message) *msgjson.Error {
	...
	rpcErr = r.checkPrefixTrade(coins, lotSize, &limit.Prefix, &limit.Trade, limit.Rate, true)
	if rpcErr != nil {
		return rpcErr
	}
	...
	return r.submitTrade(oRecord, tunnel, coins, limit.Coins, sell, limit.Rate, limit.RedeemSig, limit.Serialize())
}
...
func (r *OrderRouter) handleMarket(user account.AccountID, msg *msgjson.Message) *msgjson.Error {
	...
	return r.submitTrade(oRecord, tunnel, assets, market.Coins, sell, 0, market.RedeemSig, market.Serialize())
}

func (r *OrderRouter) sufficientBalance(user account.AccountID, ord order.Order, accountAddr string, assetInfo *dex.Asset, tunnel MarketTunnel) bool {
	...
	return r.accountBalancer.CheckBalance(accountAddr, assetID, fundingQty, fundingLots, redeems)
}

func (r *OrderRouter) submitTrade(oRecord *orderRecord, tunnel MarketTunnel, assets *assetSet,
	...
	// If either of the assets are account based, we need to do some balance
	// checks.
	...
	if isAccountFunded {
		...
		if len(coins[0].PubKeys) != 1 || len(coins[0].Sigs) != 1 || len(coins[0].Redeem) > 0 {
			log.Errorf("user %s submitted an %s-funded order with %d pubkeys, %d sigs, redeem script length %d",
				user, assets.funding.Symbol, len(coins[0].PubKeys), len(coins[0].Sigs), len(coins[0].Redeem))
			return msgjson.NewError(msgjson.OrderParameterError, "account-type asset funding requires exactly one coin ID")
		}

		fundingCoin := coins[0]
		acctAddr := string(fundingCoin.ID)
		pubKey := fundingCoin.PubKeys[0]
		sig := fundingCoin.Sigs[0]
		...
		if !r.sufficientBalance(user, oRecord.order, acctAddr, &assets.funding.Asset, tunnel) {
	...
	// received amount.
	if receivingBalancer, is := assets.receiving.Backend.(asset.AccountBalancer); is {
		if redeemSig == nil {
		...
		acctAddr := oRecord.order.Trade().Address
		...
		if !r.sufficientBalance(user, oRecord.order, acctAddr, &assets.receiving.Asset, tunnel) {
			return msgjson.NewError(msgjson.FundingError, "insufficient balance")
		}
	}

	if isAccountFunded {
		// Nothing left to do. Submit the order right away.
...
func (r *OrderRouter) checkPrefixTrade(assets *assetSet, lotSize uint64, prefix *msgjson.Prefix,
	...
	// balancer := assets.funding.Backend.(asset.AccountBalancer)
	// qty, lots, redeems := r.matchNegotiator.PendingStats(user, assets.funding.ID)
	// for _, tunnel := range r.tunnels {
	// 	switch {
	// 	case tunnel.Base() == assets.funding.ID || tunnel.Quote() == assets.funding.ID:
	// 		newQty, newLots := tunnel.UserPending(user, assets.funding.ID)
	// 		qty += newQty
	// 		lots += newLots
	// 	case tunnel.Base() == assets.receiving.ID || tunnel.Quote() == assets.receiving.ID:
	// 		_, newRedeems := tunnel.UserPending(user, assets.receiving.ID)
	// 		redeems += int(newRedeems)
	// 	}
	// }

	// if trade.Side == msgjson.SellOrderNum || prefix.OrderType == msgjson.MarketOrderNum {
	// 	qty += trade.Quantity
	// 	lots += qty / lotSize
	// } else {
	// 	quoteQty := calc.BaseToQuote(rate, trade.Quantity)
	// 	qty += quoteQty
	// 	mktName, _ := dex.MarketName(prefix.Base, prefix.Quote)
	// 	tunnel := r.tunnels[mktName]
	// 	midGap := tunnel.MidGap()
	// 	if midGap > 0 {
	// 		quoteLotSize := calc.QuoteToBase(midGap, quoteQty)
	// 		lots += trade.Quantity / quoteLotSize
	// 	} else {
	// 		lots++
	// 	}
	// }

	// swapReq := calc.RequiredOrderFunds(qty, 0, lots, &assets.funding.Asset)
	// redeemReq := uint64(redeems) * assets.funding.MaxFeeRate * assets.funding.RedeemSize / calc.RateEncodingFactor

	// reqFunds := redeemReq + swapReq

	// if bal, err := balancer.AccountBalance(trade.Address); err != nil {
	// 	log.Errorf("AccountBalance error: %v", err)
	// 	return msgjson.NewError(msgjson.RPCInternalError, "internal error 3865109")
	// } else if bal < reqFunds {
	// 	return msgjson.NewError(msgjson.FundingError,
	// 		"insufficient funds. required = %d %s @ %d lots and %d redeems with max fee rate of %d = %d + %d = %d, balance = %d",
	// 		qty, assets.funding.Symbol, lots, redeems, assets.funding.MaxFeeRate, swapReq, redeemReq, reqFunds, bal)
	// }
	// return nil
}
