...
export default class MarketsPage extends BasePage {
  ...
  constructor (main: HTMLElement, pageParams: MarketsPageParams) {
    ...
    bind(page.maxOrd, 'click', () => {
      if (this.isSell()) {
        const rate = this.market.dex ? 0 : this.adjustedRate()
        const maxSell = this.market.maxSells[rate]
        if (!maxSell) return
  ...
  updateOrderBttnState () {
    ...
    if (sell) {
      ...
      const maxSell = mkt.maxSells[mkt.dex ? 0 : this.adjustedRate()]
      if (maxSell) {
        this.setOrderBttnEnabled(orderQty <= maxSell.swap.value, intl.prep(intl.ID_ORDER_BUTTON_SELL_BALANCE_ERROR))
      }
      return
    }
  ...
  */
  ...
  preBuy () {
    ...
    if (mkt.maxBuys[rate]) {
      this.setMaxOrder(mkt.maxBuys[rate])
      this.updateOrderBttnState()
      return
    }
  ...
  setMaxOrder (maxOrder: MaxOrderEstimate | null) {
    const page = this.page
    if (this.maxLoaded) {
      this.maxLoaded()
      this.maxLoaded = null
    ...
    let lots = 0
    if (maxOrder) lots = maxOrder.swap.lots
    ...
    if (lots === 0) {
      ...
      if (haveQty) {
        if (fromAsset.token) {
          ...
          if (feeAvail < swap.feeReservesPerLot) {
            Doc.show(page.maxZeroNoFees)
            page.maxZeroNoFeesTicker.textContent = unitInfo.conventional.unit
            page.maxZeroMinFees.textContent = Doc.formatCoinValue(swap.feeReservesPerLot, unitInfo)
          }
        ...
        const fundedLots = fromAsset.wallet.balance.available / (lotSize + swap.feeReservesPerLot)
    ...
    page.maxFromAmt.textContent = Doc.formatCoinValue(swap.value || 0, fromAsset.unitInfo)
    page.maxFromTicker.textContent = fromAsset.unitInfo.conventional.unit
    // Could subtract the swap.redemptionFees here.
    ...
    // page.maxToAmt.textContent = Doc.formatCoinValue((swap.value || 0) * toConversion, toAsset.unitInfo)
  ...
  validateOrder (order: TradeForm) {
    const { page, market: { dex, rateConversionFactor } } = this
    if (order.isLimit) {
      if (!order.rate) {
      ...
      // TODO: Implement min rate check for mesh.
      if (dex && order.rate < dex.cfg.minimumRate) {
        Doc.show(page.orderErr)
        const [r, minRate] = [order.rate / rateConversionFactor, dex.cfg.minimumRate / rateConversionFactor]
  ...
  handleBook (data: MarketOrderBook) {
    const { dex, baseUnitInfo, quoteUnitInfo, base: { symbol: baseSymbol }, quote: { symbol: quoteSymbol } } = this.market
    this.book = new OrderBook(data, baseSymbol, quoteSymbol)
    this.loadTable()
    ...
    const [lotSize, rateStep] = [dex?.cfg.lotsize || 1, dex?.cfg.ratestep || 1]
  ...
  setMarketBuyOrderEstimate () {
    const { page, market: mkt } = this
    if (!mkt.dex) return // No market buys on mesh anyway.
    const lotSize = mkt.dex.cfg.lotsize
    const xc = app().user.exchanges[mkt.dex.xc.host]
    const buffer = xc.markets[mkt.mktID].buybuffer
    const gap = this.midGapConventional()
    if (gap) {
      page.minMktBuy.textContent = Doc.formatCoinValue(lotSize * buffer * gap, mkt.baseUnitInfo)
    }
  }

  maxUserOrderCount (): number {
    const { host, mktID } = this.market
  ...
  refreshActiveOrders () {
    const orders = app().orders(this.market.host, marketID(this.market.base.symbol, this.market.quote.symbol))
    return this.resolveUserOrders(orders)
  }

  resolveUserOrders (orders: Order[]) {
    const { page, metaOrders, market } = this
    const rateStep = market.dex?.cfg.ratestep || 1
    ...
    for (const mord of sortedOrders) {
      ...
      else rateStr = Doc.formatRateFullPrecision(ord.rate, market.baseUnitInfo, market.quoteUnitInfo, rateStep)
  ...
  */
  marketOrderRateString (ord: Order, mkt: CurrentMarket) :string {
    ...
    let rateStr = Doc.formatRateFullPrecision(OrderUtil.averageRate(ord), mkt.baseUnitInfo, mkt.quoteUnitInfo, mkt.dex?.cfg.ratestep || 1)
  ...
  */
  ...
  handleBookOrderRoute (data: BookUpdate) {
    ...
    if (data.host !== this.market.host || data.marketID !== this.market.mktID) return
  ...
  handleCandlesRoute (data: BookUpdate) {
    ...
    this.candleChart.setCandles(data.payload, this.market.dex?.cfg.ratestep || 1, this.market.baseUnitInfo, this.market.quoteUnitInfo)
  }
  ...
  handleCandleUpdateRoute (data: BookUpdate) {
    if (data.host !== this.market.host) return
  ...
  handlePriceUpdate (note: SpotPriceNote) {
    ...
    if (note.host === this.market.host && note.spots[this.market.mktID]) {
      this.setCurrMarketPrice()
    }
  ...
  async handleBondUpdate (note: BondNote) {
    ...
    if (!this.market) return // This note can arrive before the market is set.
    if (dexAddr !== this.market.host) return
    ...
    if (this.market.dex) this.market.dex.xc = app().exchanges[dexAddr]
    this.setRegistrationStatusVisibility()
    this.updateReputation()
  }

  updateReputation () {
    const { page, market: { dex, host, base: { unitInfo: bui }, quote: { unitInfo: qui } } } = this
    if (!dex) return // No reputation shown for mesh yet
    const { cfg: mkt } = dex
  ...
  anyRate (): [number, number, number] {
    const { dex, base: { id: baseID }, quote: { id: quoteID }, rateConversionFactor, bookLoaded } = this.market
    if (bookLoaded) {
      const midGap = this.midGap()
      if (midGap) return [midGap * OrderUtil.RateEncodingFactor, midGap, this.midGapConventional() || 0]
    }
    const spotRate = dex?.cfg.spot?.rate
    if (spotRate) return [spotRate, spotRate / OrderUtil.RateEncodingFactor, spotRate / rateConversionFactor]
  ...
  handleEpochNote (note: EpochNote) {
    ...
    if (!this.market) return // This note can arrive before the market is set.
    if (note.host !== this.market.host || note.marketID !== this.market.mktID) return
    if (this.book) {
  ...
  async createWallet () {
    ...
    Doc.hide(this.page.forms)
    const mkt = this.market
    if (mkt.base.id === asset.id) mkt.base = asset
    else if (mkt.quote.id === asset.id) mkt.quote = asset
  ...
  lotChanged () {
    ...
    const lotSize = this.market.dex?.cfg.lotsize || 1
  ...
  orderTableRow (orderBin: MiniOrder[]): OrderRow {
    ...
    const { baseUnitInfo, quoteUnitInfo, rateConversionFactor, dex } = this.market
    const rateStep = dex?.cfg.ratestep || 1
  ...
  candleDurationSelected (dur: string) {
    if (!this.market?.dex?.xc.candleDurs.includes(dur)) dur = oneHrBinKey
    this.candleDur = dur
    this.loadCandles()
  ...
  loadCandles () {
    ...
    const { candleCaches, dex, baseUnitInfo, quoteUnitInfo } = this.market
    if (!dex) return console.error('Cannot load candles for mesh')
    const cache = candleCaches[this.candleDur]
    if (cache) {
      // this.depthChart.hide()
      // this.candleChart.show()
      this.candleChart.setCandles(cache, dex?.cfg.ratestep, baseUnitInfo, quoteUnitInfo)
      return
    }
    this.requestCandles()
  }
  ...
  requestCandles (candleDur?: string) {
    ...
    const { host, base: { id: baseID }, quote: { id: quoteID } } = this.market
    ws.request('loadcandles', { host: host, base: baseID, quote: quoteID, dur: candleDur || this.candleDur })
  }
...
class BalanceWidget {
  ...
  setWallets (baseID: number, quoteID: number) {
    ...
    this.setBalanceVisibility(true /* this.dex.connectionStatus === ConnectionStatus.Connected */) // TODO
  }
