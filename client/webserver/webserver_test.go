...
func TestAPI_RateSourceSettings(t *testing.T) {
	s, tCore, shutdown, err := newTServer(t, false)
	if err != nil {
		t.Fatalf("error starting server: %v", err)
	}
	...
	// Test enabling exchange rate sources.
	enableTests := []struct {
		...
		source:  "binance",
		wantErr: errors.New("cannot enable unkown exchange rate source"),
		want:    `{"ok":false,"msg":"cannot enable unkown exchange rate source"}`,
	}, {
	...
	for _, test := range enableTests {
		...
		ensureResponse(t, s.apiRateSourceSettings, test.want, reader, writer, body, nil)
	}

	// Test disabling exchange rate sources.
	disableTests := []struct {
		name, source, want string
		wantErr            error
	}{{
		name:    "Invalid rate source",
		source:  "binance",
		wantErr: errors.New("cannot disable unkown exchange rate source"),
		want:    `{"ok":false,"msg":"cannot disable unkown exchange rate source"}`,
	}, {
		name:   "ok valid source",
		source: "messari",
		want:   `{"ok":true}`,
