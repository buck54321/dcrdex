...
var (
	...
	walletOpts = []*asset.ConfigOption{
		...
		// TODO: Uncomment when staking is ready.
		// {
		// 	Key:         "vspurl",
		// 	DisplayName: "Voting Service Provider URL",
		// 	Description: "The url of the voting service provider to use when purchasing tickets. " +
		// 		"Can only be used by SPV wallets.",
		// 	DefaultValue: "",
		// },
	}
...
func NewWallet(cfg *asset.WalletConfig, logger dex.Logger, network dex.Network) (*ExchangeWallet, error) {
	...
	case walletTypeSPV:
		...
		if walletCfg.VSPURL != "" {
			if err := dcr.SetVSP(walletCfg.VSPURL); err != nil {
				logger.Errorf("unable to retrieve vsp data: %v", err)
			}
		}
	default:
