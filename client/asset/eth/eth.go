...
type ethFetcher interface {
	...
	nextNonce(ctx context.Context) (confirmed, next *big.Int, err error)
}
...
func (w *ETHWallet) Connect(ctx context.Context) (_ *sync.WaitGroup, err error) {
	...
	confirmedNonce, nextNonce, err := w.node.nextNonce(ctx)
	if err != nil {
...
func nonceIsSane(pendingTxs []*extendedWalletTx, pendingNonceAt *big.Int) error {
	...
	for i, pendingTx := range pendingTxs {
		...
		if age >= txAgeOut && pendingTx.BlockNumber == 0 {
...
func (w *baseWallet) checkPendingTxs() {
	...
	if !w.recoveryRequestSent && len(findMissingNonces(w.confirmedNonceAt, w.pendingNonceAt, w.pendingTxs)) != 0 {
		...
		w.requestAction(actionTypeMissingNonces, w.missingNonceID(), nil, nil)
	}
	...
	for i, pendingTx := range w.pendingTxs {
		...
		if time.Since(pendingTx.actionIgnored) < txAgeOut {
			// The asked us to keep waiting.
			continue
		}
		age := pendingTx.age()
		// Did we have any abnormally ordered confirmed nonces?
		...
		if txCap.Cmp(baseRate) < 0 {
			...
			req := newLowFeeeNote(*pendingTx.WalletTransaction, dexeth.WeiToGweiCeil(maxFees))
			pendingTx.actionRequested = true
...
func (w *baseWallet) missingNonceID() string {
