...
func (btc *ExchangeWallet) checkForNewBlocks() {
	...
	// height + 1.
	var startPoint *chainhash.Hash
	var startPointErr error
	prevTipBlock, err := btc.node.getBlockHeader(prevTip.hash)
	switch {
	case err != nil:
	...
	if startPointErr != nil {
		...
		go btc.tryRedemptionRequests(startPoint, reqs)
	}
}
