...
// isActiveBondAsset indicates if a wallet is needed for bonding on any
// configured DEX. includeLive should be set to consider all existing unspent
// bonds that need to be refunded in the future.
...
func (c *Core) bondKey() *hdkeychain.ExtendedKey {
	c.loginMtx.Lock()
	...
	// clone it
	xPriv, _ := hdkeychain.NewKeyFromString(c.bondXPriv.String(), &keygen.RootKeyParams{})
	return xPriv
}
